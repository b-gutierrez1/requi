    ...
    public function esAutorizadorPago($email)
    {
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                WHERE LOWER(amp.autorizador_email) = LOWER(?)
                   OR LOWER(amp.autorizador_email) IN (
                       SELECT LOWER(u.email)
                       FROM usuarios u
                       WHERE (LOWER(u.email) = LOWER(?) OR LOWER(u.azure_email) = LOWER(?))
                         AND u.email IS NOT NULL
                   )
                   OR LOWER(amp.autorizador_email) IN (
                       SELECT LOWER(u.azure_email)
                       FROM usuarios u
                       WHERE (LOWER(u.email) = LOWER(?) OR LOWER(u.azure_email) = LOWER(?))
    ...
    public function esAutorizadorCuenta($email)
    {
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                WHERE LOWER(acc.autorizador_email) = LOWER(?)
                   OR LOWER(acc.autorizador_email) IN (
                       SELECT LOWER(u.email)
                       FROM usuarios u
                       WHERE (LOWER(u.email) = LOWER(?) OR LOWER(u.azure_email) = LOWER(?))
                         AND u.email IS NOT NULL

...
<div class="container py-4" style="max-width: 1200px;">
    ...
    <div class="quick-actions">
        ...
        <?php $count_auth_pendientes = count($autorizaciones_pendientes ?? []) + count($revisiones_pendientes ?? []); ?>
        <a href="<?= url('/autorizaciones') ?>" class="quick-action" style="position: relative;">
            <?php if ($count_auth_pendientes > 0): ?>
            <span style="position:absolute;top:12px;right:12px;background:#f59e0b;color:#fff;font-size:0.75rem;font-weight:700;min-width:22px;height:22px;border-radius:11px;display:flex;align-items:center;justify-content:center;padding:0 5px;box-shadow:0 2px 6px rgba(0,0,0,0.2);">
                <?= $count_auth_pendientes ?>
            </span>
            <?php endif; ?>
            ...
            <p class="small text-muted mb-0">
                <?php if ($count_auth_pendientes > 0): ?>
                    <?= $count_auth_pendientes ?> pendiente<?= $count_auth_pendientes > 1 ? 's' : '' ?>
                <?php else: ?>

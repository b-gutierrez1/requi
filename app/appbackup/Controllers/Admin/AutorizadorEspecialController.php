    ...
    public function storeMetodoPago()
    {
        try {
            ...
            if ($existente) {
                ...
                Redirect::back()
                    ->withError("El método de pago '{$descripcionMetodo}' ya tiene asignado el autorizador '{$existente['autorizador_email']}'. Para cambiarlo, edite el registro existente.")
                    ->withInput()
                    ->send();
                return;
            }
    ...
    public function updateMetodoPago($id)
    {
        try {
            ...
            $stmtCheck = Model::getConnection()->prepare(
                "SELECT id FROM autorizadores_metodos_pago WHERE metodo_pago = ? AND id != ?"
            );
            $stmtCheck->execute([$metodoPago, $id]);
            if ($stmtCheck->fetch()) {
                Redirect::back()->withError("El método de pago '{$metodoPago}' ya tiene otro autorizador asignado.")->withInput()->send();
                return;
            }

...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Agregar event listener para el cambio de unidad de negocio en la primera fila
    document.querySelectorAll('.distribucion-row').forEach(row => {
        const unidadNegocioSelect = row.querySelector('select[name*="[unidad_negocio_id]"]');
        if (unidadNegocioSelect) {
            unidadNegocioSelect.addEventListener('change', function() {
                if (window.calculadorAutomatico) {
                    window.calculadorAutomatico.actualizarFacturaPorUnidadNegocio(row);
                }
            });
        }
    });
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            Object.keys(distributionData).forEach(index => {
                ...
                if (dist.unidad_negocio_id && (!dist.cuenta_contable_id || dist.cuenta_contable_id === '')) {
                    console.log(`❌ Distribution ${index}: Has business unit but missing account ID`);
                    issuesFound++;
                }
...
function buscarCentroCostoPorNombre(nombreUnidad) {
    const unidadesNegocio = [
        <?php if (!empty($centros_costo)): ?>
            <?php foreach ($centros_costo as $unidad): ?>
                {
                    id: '<?= $unidad['id'] ?>',
